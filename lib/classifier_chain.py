...
# <=====================================================================================>
# load the data
df = joblib.load('/Users/pengfeiwang/Desktop/f/fisheries/data/data2.pkl','rb')
...
x2 = test_df.ix[:,1:]
y2 = test_df['label']
tr_x, te_x, tr_y, te_y = train_test_split(x2, y2, test_size=0.8, random_state=1)
...
for i,j in enumerate(clf):
    result = j.predict_proba(testx)
    t[i] = [prob[1] for prob in result]

row_max = t.apply(lambda x: x.max(),axis=1).tolist()
for i in range(t.shape[0]):
	t.ix[i,:] = [1 if item==row_max[i] else 0 for item in t.ix[i,:].tolist()]

result = clf_logis.predict_proba(t)
# result to csv
